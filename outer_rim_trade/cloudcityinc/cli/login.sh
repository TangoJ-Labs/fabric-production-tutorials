...
if [ $USERNAME = "" ] || [ $PASSWORD = "" ] || [ $CACERT = "" ]; then
    ...
    if [ ! -d $FABRIC_CA_CLIENT_HOME/msp ]; then
        ...
        cp $FABRIC_CA_CLIENT_HOME/msp/signcerts/cert.pem $FABRIC_CA_CLIENT_HOME/msp/signcerts/$USERNAME@cloudcityinc-cert.pem #MSP CORRECTION
        # For certfileuserstore (github.com/hyperledger/fabric-sdk-go/pkg/msp/certfileuserstore.go):
        cp $FABRIC_CA_CLIENT_HOME/msp/signcerts/cert.pem $FABRIC_CA_CLIENT_HOME/msp/signcerts/$USERNAME@cloudcityincMSP-cert.pem #MSP CORRECTION
    fi
