...
RUN_LOGFILE=/shared/logs/run.log
RUN_SUMFILE=/shared/logs/run.sum
RUN_SUCCESS_FILE=/shared/logs/run.success
RUN_FAIL_FILE=/shared/logs/run.fail
...
function chaincodeQuery {
   ...
   while test "$(($(date +%s)-starttime))" -lt "$QUERY_TIMEOUT"; do
      sleep 1
      peer chaincode query -C mychannel -n mycc -c '{"Args":["query","a"]}' >& /shared/logs/cctest.log
      VALUE=$(cat /shared/logs/cctest.log | awk '/Query Result/ {print $NF}')
      if [ $? -eq 0 -a "$VALUE" = "$1" ]; then
         ...
         VALUE=$(cat /shared/logs/cctest.log | egrep '^[0-9]+$')
         if [ $? -eq 0 -a "$VALUE" = "$1" ]; then
            logr "Query of channel 'mychannel' on peer 'org1-peer0' was successful"
            set -e
            return 0
   ...
   cat /shared/logs/cctest.log
   cat /shared/logs/cctest.log >> $RUN_SUMFILE
